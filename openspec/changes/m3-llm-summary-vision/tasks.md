...
- [ ] 9.4 测试场景 4：LLM 超时 → 笔记仍含字幕（summary 占位，summary_status=failed）
- [ ] 9.5 测试场景 5：VLM 禁用（`vision.enabled: false`）→ 笔记只含总结，关键帧段为占位
- [ ] 9.6 性能基准：5 条视频串行，平均端到端 ≤ 4 分钟/条（含 LLM 调用）
- [ ] 9.7 更新 `docs/m2/KNOWLEDGE.md`：新增 LLM prompt 模板、VLM 选型对比、4070S 显存预算表
